...
class nnUNetTrainer(NetworkTrainer):
    ...
    def get_basic_generators(self):
        ...
        if self.threeD:
            ...
            # traine-data  直接作为valid集
            # dl_tr = DataLoader3D(self.dataset_tr, self.basic_generator_patch_size, self.patch_size, self.batch_size,
            #                      False, oversample_foreground_percent=self.oversample_foreground_percent,
            #                      pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
            # dl_val = DataLoader3D(self.dataset_tr, self.patch_size, self.patch_size, self.batch_size, False,
            #                       oversample_foreground_percent=self.oversample_foreground_percent,
            #                       pad_mode="constant", pad_sides=self.pad_all_sides, memmap_mode='r')
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        current_mode = self.network.training       #这一步是加载训练模型吗？---是的
        self.network.eval()
        ...
        for k in self.dataset_val.keys():
            ...
            if overwrite or (not isfile(join(output_folder, fname + ".nii.gz"))) or \
                ...
                #valid对应选择拼接不同的ep
                # data = np.concatenate((data[:3],data[4][np.newaxis,:],data[-1,...][np.newaxis,:]),axis=0)
    ...
    def finish_online_evaluation(self):
        ...
        global_dc_per_class = [i for i in [2 * i / (2 * i + j + k) for i, j, k in
                               ...
                               if not np.isnan(i)]
        #计算每个case的recall
        global_recall_per_class = [i for i in [2 * i / (2 * i  + k) for i,  k in
                                           zip(self.online_eval_tp, self.online_eval_fn)]
                               if not np.isnan(i)]


        self.all_val_eval_metrics.append(np.mean(global_dc_per_class))#将所有的epcoh的valid dice都存放在一个列表中

        self.print_to_log_file("Average global foreground Dice:", str(global_dc_per_class))
        self.print_to_log_file("(interpret this as an estimate for the Dice of the different classes. This is not "
                               "exact.)") 
        self.print_to_log_file("global_recall_per_class:", str(global_recall_per_class))
