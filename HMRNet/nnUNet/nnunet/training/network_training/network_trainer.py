...
class NetworkTrainer(object):
    ...
    def load_checkpoint(self, fname, train=True):
        ...
        self.load_checkpoint_ram(saved_model, train)    #加载模型的调用语句
    ...
    def load_checkpoint_ram(self, checkpoint, train=True):             #加载模型的具体操作函数
        """
    ...
    def run_training(self):
        ...
        if not self.was_initialized:
            self.initialize(True)#初始化所有的网络参数

        while self.epoch < self.max_num_epochs:#进入网络训练
            ...
            with torch.no_grad():
                ...
                for b in range(self.num_val_batches_per_epoch):
                    l = self.run_iteration(self.val_gen, False, True)  #进行迭代并且进行online_validation
                    val_losses.append(l)
    ...
    def update_eval_criterion_MA(self):
        ...
        if self.val_eval_criterion_MA is None:
            if len(self.all_val_eval_metrics) == 0:
                ...
                self.val_eval_criterion_MA = self.all_val_eval_metrics[-1] #self.all_val_eval_metrics[-1]#第一步将最近的epoch的valid dice赋值过去
        else: #当运行第一个epoc之后，运行此操作
            if len(self.all_val_eval_metrics) == 0:  
                """
    ...
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            # initialize them
            if self.best_MA_tr_loss_for_patience is None:    #这一步操作被采用
            ...
            self.print_to_log_file("Best valid  dice is :%.4f0" % self.best_val_eval_criterion_MA)   
            if self.val_eval_criterion_MA > self.best_val_eval_criterion_MA:     #判断valid_dice最高的模型并保存
                self.best_val_eval_criterion_MA = self.val_eval_criterion_MA             
    ...
    def on_epoch_end(self):
        ...
        self.update_eval_criterion_MA()#将最新的epoch的valid dice赋值给eval_criterion_MA
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if run_online_evaluation:# 此处的valid dice 不是真正的valid dice因为计算不够准确(计算的方式不同，不是采用的sliding_window方式)
